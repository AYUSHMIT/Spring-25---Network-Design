...
from file_transfer import send_file, receive_file  # Ensure receive_file is imported

def start_receiver():
    # Start the receive_file function in a separate thread
    threading.Thread(target=receive_file_with_status, daemon=True).start()

def receive_file_with_status():
    try:
        update_fsm_state("Receiving file...")
        start_time = time.time()
        receive_file(update_fsm_state, option=1, error_rate=0.0)  # Pass the required arguments
        end_time = time.time()
...
def send_file_thread(file_path, retry_count=3):
    try:
        ...
        start_time = time.time()
        send_file(file_path, update_fsm_state, option=1, error_rate=0.0)  # Pass the required arguments
        end_time = time.time()
        transfer_time = end_time - start_time
...
receive_button = tk.Button(window, text="Receive File", command=start_receiver, font=("Arial", 14))
